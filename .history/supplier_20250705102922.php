...
<body style="background:linear-gradient(135deg,#e0f2fe 0%,#f0f9ff 100%); min-height:100vh;">
                    ...
                    <div class="col-12">
                        <div class="card border-0 rounded-4 shadow-sm" style="background:#f8fbff;">
                            <div class="card-header bg-white border-0 rounded-top-4 d-flex align-items-center justify-content-between" style="border-bottom:1px solid #e0e7ef; background:#e0f2fe;">
                                ...
                                <!-- Catatan: File alternatif-simpan.php sudah tidak diperlukan dan bisa dihapus -->
                            </div>
                            <div class="card-content">
                                ...
                                <div class="table-responsive rounded-4 overflow-hidden">
                                    <table class="table table-hover align-middle mb-0" style="background:#fff;">
                                        ...
                                        <tbody>
                                        <?php
                                        $sql = 'SELECT id_supplier, name, foto FROM supplier';
                                        $result = $db->query($sql);
                                        $i = 0;
                                        while ($row = $result->fetch_object()) {
                                            ...
                                            if (!empty($row->foto)) {
                                                echo "<img src='" . htmlspecialchars($row->foto) . "' alt='Foto' style='width:40px;height:40px;object-fit:cover;border-radius:50%;border:1.5px solid #38b6ff;background:#e0f2fe;'> ";
                                            } else {
                                                echo "<span class='d-inline-block rounded-circle bg-light border border-2 border-sky' style='width:40px;height:40px;background:#e0f2fe;'></span> ";
                                            }
                                            echo "</td>\n";
                                            ...
                                            echo "<a href='supplier/supplier-edit.php?id=" . $row->id_supplier . "' class='btn btn-outline-primary btn-sm px-3 py-1 fw-bold d-inline-flex align-items-center' style='border-radius:1.5rem;background:#f0f9ff;border:1.5px solid #38b6ff;color:#0369a1;gap:4px;'><i class='bi bi-pencil-square' style='margin-right:0;'></i><span>Edit</span></a>";
                                            echo "<a href='supplier/supplier-hapus.php?id=" . $row->id_supplier . "' class='btn btn-outline-danger btn-sm px-3 py-1 fw-bold d-inline-flex align-items-center' style='border-radius:1.5rem;background:#f0f9ff;border:1.5px solid #e11d48;color:#e11d48;gap:4px;' onclick=\"return confirm('Yakin ingin menghapus data ini?')\"><i class='bi bi-trash' style='margin-right:0;'></i><span>Hapus</span></a>";
                                            echo "</div>";
                                            echo "    </td>\n";
                                            echo "  </tr>\n";
                                        }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
        ...
        if ($name && $foto) {
            ...
            $stmt->execute();
            echo '<script>location.href="supplier/supplier.php";</script>';
            exit;
        } else {

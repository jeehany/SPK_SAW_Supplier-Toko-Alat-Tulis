                            ...
                            <div class="card-content">
                                ...
                                <div class="table-responsive rounded-4 overflow-hidden">
                                    <table class="table table-hover align-middle mb-0" style="background:#fff;">
                                        ...
                                        <tbody>
                                        <?php
                                        $sql = 'SELECT id_supplier, name, deskripsi, foto, alamat, email, telepon, dokumen, jenis_dokumen FROM supplier ORDER BY id_supplier ASC';
                                        $result = $db->query($sql);
                                        while ($row = $result->fetch_object()) {
                                            ...
                                            if (!empty($row->dokumen)) {
                                                echo "<a href='../{$row->dokumen}' target='_blank' class='btn btn-sm btn-outline-primary' style='border-radius:2rem;'>";
                                                echo "<i class='bi bi-file-earmark-text me-1'></i>";
                                                echo htmlspecialchars($row->jenis_dokumen);
                                                echo "</a>";
                                            } else {
                                                echo "<span class='badge bg-secondary'>No File</span>";
                                            }
                                            echo "</td>\n";
                                            // Kolom aksi
                                            echo "    <td class='text-center'>";
                                            ...
                                            echo "<div class='d-none supplier-detail' id='supplier-detail-{$row->id_supplier}' data-nama='" . htmlspecialchars($row->name ?? '') .
                                                ...
                                                "' data-dokumen='" . htmlspecialchars($row->dokumen ?? '') .
                                                "' data-jenis-dokumen='" . htmlspecialchars($row->jenis_dokumen ?? '') .
                                                "'></div>";
                                            echo "    </td>\n";
                                            echo "  </tr>\n";
                                        }
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="modal-body py-4 px-4" style="max-height:65vh;overflow-y:auto;">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">Upload foto supplier/logo perusahaan</small>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold text-dark">Jenis Dokumen</label>
                            <select name="jenis_dokumen" class="form-select form-select-lg rounded-3">
                                <option value="">- Pilih -</option>
                                <option value="Sertifikat Toko">Sertifikat Toko</option>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
        ...
        if (isset($_FILES['dokumen']) && $_FILES['dokumen']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($dokumen_ext, $allowed)) {
                $dokumen_name = 'uploads/dokumen/doc_' . time() . '_' . rand(100,999) . '.' . $dokumen_ext;
                if (!is_dir('../uploads/dokumen')) mkdir('../uploads/dokumen', 0777, true);
                move_uploaded_file($_FILES['dokumen']['tmp_name'], '../' . $dokumen_name);
                $dokumen = $dokumen_name;
            }
        }
        
        if ($name && $foto) {
            $stmt = $db->prepare('INSERT INTO supplier (name, deskripsi, alamat, email, telepon, foto, dokumen, jenis_dokumen) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        modal.addEventListener('show.bs.modal', function (event) {
          ...
          if (detail) {
            ...
            var html = `
              ...
              ${dokumen ? `<div class='mb-2'>
                <span class='fw-bold text-dark'>Dokumen ${jenisDokumen}:</span><br>
                <a href='../${dokumen}' target='_blank' class='btn btn-sm btn-outline-primary mt-1' style='border-radius:2rem;'>
